...
class CosmeticEditProposal(models.Model):
    STATUS_CHOICES = [
        ('PENDING', 'Pending'),
        ('APPROVED', 'Approved'),
        ('REJECTED', 'Rejected'),
    ...
    cosmetic = models.ForeignKey(Cosmetic, on_delete=models.CASCADE, related_name='edit_proposals')
    ...
    def __str__(self):
        return f"Edit proposal for {self.cosmetic} ({self.status})"
